...
CREATE TABLE IF NOT EXISTS elective_terms (
    ...
    year SMALLINT NOT NULL,                   -- พ.ศ. เช่น 2569
    semester SMALLINT NOT NULL,               -- 1 = ต้น, 2 = ปลาย, 3 = ฤดูร้อน
    status TEXT NOT NULL DEFAULT 'current',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uq_elective_terms_year_semester UNIQUE (year, semester),
    CONSTRAINT ck_elective_terms_year CHECK (year BETWEEN 2500 AND 2700),
...
CREATE TABLE IF NOT EXISTS elective_sessions (
    ...
    -- วิชาเดียวลงคาบเดียวกันสองครั้งไม่ได้
    CONSTRAINT uq_elective_sessions_course_slot UNIQUE (elective_id, slot),
    -- ห้องหนึ่ง คาบหนึ่ง ในเทอมหนึ่ง มีได้คลาสเดียว - หน้าเว็บปฏิเสธตั้งแต่ตอน
    -- ลากแล้ว (assertPlacement) ตรงนี้คือเส้นสุดท้ายที่กันคำสั่งที่ไม่ผ่านหน้าจอ
    -- room_id ที่เป็น NULL ไม่ชนกันเองตามปกติของ UNIQUE วิชาออนไลน์หลายวิชาจึง
    -- อยู่คาบเดียวกันได้
    CONSTRAINT uq_elective_sessions_room_slot UNIQUE (term_id, room_id, slot),
