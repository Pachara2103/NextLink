...
def direct_enabled():
    if config.LINE_DATA_MODE not in {"shared", "direct"}:
        raise BadRequestError(message="LINE_DATA_MODE must be shared or direct; API mode has been retired")
...
def read_messages(group_id, versions):
    ...
    with line_db.get_connection() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
        source_identity(cur)
        cur.execute("""SELECT message_id,source_version,text_content
