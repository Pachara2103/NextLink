    ...
    with conn.cursor() as cur:
        for item in page["items"]:
            ...
            if item["entity"] == "group":
                ...
                # Preserve the old is_read baseline once; later source changes
                # do not reopen messages already acknowledged by NextLink.
                ...
                cur.execute("""INSERT INTO line_message_processing
                    (message_id,group_id,source_version,eligible,processed_version)
                    VALUES (%s,%s,%s,%s,%s)
                    ...
                    (item["entity_id"],item["message_group_id"],version,item["eligible"],
                     version if legacy_read else None,bootstrap_watermark))
            else:
