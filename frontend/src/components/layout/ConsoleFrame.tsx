...
export function ConsoleFrame({ active, onNavigate, fullHeight = false, wide = false, children }: {
  ...
  // Dashboard pages supply their own main landmark.
  const Content = active === "dashboard" ? "div" : "main";
  return <>
    ...
    <div className={fullHeight ? "relative flex h-screen overflow-hidden" : "relative flex min-h-screen"}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <ToastHost />
        <ServerStatusBanner />
        <Content className={fullHeight ? "flex min-h-0 flex-1 flex-col" : wide ? "min-w-0 flex-1" : "mx-auto w-full max-w-[1200px] flex-1 px-5 py-7 sm:px-8"}>
