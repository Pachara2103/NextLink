...
import { slotLabelWithTime, type SlotId } from "@/features/elective-plan/lib/slots.ts";
...
export function BookingDialog(props: Omit<BookingDialogProps, "target"> & { target: BookingTarget | null }) {
  return props.target ? <BookingDialogForm {...props} target={props.target} key={`${props.target.room.id}/${props.target.slotId}`} /> : null;
}

function BookingDialogForm({
  target,
  rooms,
  onSave,
  onClose,
