...
import { ConflictPanel } from "@/features/elective-plan/components/conflict-panel";
...
export function PlanOverview({ payload }: { payload: PlanPayload }) {
  ...
  const [, setHolding] = useState<PlanCourse | null>(null);
  ...
  const applySuggestion = async (suggestion: Suggestion) => {
    ...
    if (suggestion.kind === "ASK_MORE_AVAILABILITY") {
      ...
      router.push(`/elective-plan/courses?${params.toString()}`);
      return;
    }
    ...
    >
      ...
      <section className="panel matrix-panel" ref={boardRef}>
        <div className="panel-heading">
          ...
          <div className="table-heading-actions">
            <Link className="text-button" href="/elective-plan/courses/list">ดูรายวิชาทั้งหมด</Link>
