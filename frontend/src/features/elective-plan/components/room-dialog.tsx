...
import { roomDraftFrom, validateRoomDraft, type RoomDraft } from "@/features/elective-plan/lib/rooms.ts";
import type { PlanRoom } from "@/features/elective-plan/lib/plan-types.ts";
...
export function RoomDialog(props: Omit<RoomDialogProps, "target"> & { target: RoomFormTarget | null }) {
  return props.target ? <RoomDialogForm {...props} target={props.target} key={props.target.room?.id ?? "new"} /> : null;
}
...
function RoomDialogForm({
  ...
  const [draft, setDraft] = useState<RoomDraft>(() => target.room ? roomDraftFrom(target.room) : NEW_ROOM);
