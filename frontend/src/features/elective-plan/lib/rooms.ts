 ...
 *
 * The id is a URL — `/elective-plan/rooms/<id>` — and a Thai one survives a copy-paste into
...
export function makeRoomId(draft: RoomDraft, taken: Iterable<string>): string {
  ...
  const ascii = `${draft.building} ${draft.name}`
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  // A bare number makes an address that reads like a page number — `/elective-plan/rooms/406`
